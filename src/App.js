...
async function registerUser(uid, pw, initialWeights) {
const ref = doc(db, “users”, uid);
const snap = await getDoc(ref);
...
const ref = doc(db, “users”, id.trim());
const snap = await getDoc(ref);
if (snap.exists()) { setError(“이미 사용 중인 아이디예요.”); return; }
...
setLoading(true);
try {
const res = await registerUser(pendingUid, pendingPw, initWeights);
