...
points: [
“등받이에 허리를 완전히 밀착시켜요”,
“발판에 발을 어깨 너비로 올리고 발끝은 약간 바깥으로 향해요”,
“무릎이 발끝 방향과 일치하도록 해요”,
“천천히 무릎을 90도까지 구부리며 내려요”,
“발뒤꿈치로 발판을 밀어내듯 펴요”,
“무릎을 완전히 잠그지 말고 살짝 구부린 상태로 마무리해요”,
],
caution: “허리가 등받이에서 떨어지지 않도록 주의해요. 무릎을 완전히 잠그면 관절에 무리가 가요.”,
},
“불가리안 스플릿 스쿼트”: {
...
caution: “덤벨 중량은 양손 합산으로 입력해요. 더블 프로그레션 방식으로 8→12회 채우면 중량을 올려요. 균형 잡기 어려우면 처음엔 맨몸으로 시작하세요.”,
},
“바벨 로우”: {
target: “광배근, 승모근, 이두”,
points: [
...
A: [
{ name: “불가리안 스플릿 스쿼트”, sets: 3, minReps: 8, maxReps: 12, increment: 0, isAccessory: true, isBulgarian: true },
...
legpress: { name: “레그프레스”, sets: 5, reps: 5, increment: 5, isLegPress: true },
};
...
const EXERCISE_NAMES = [“벤치프레스”, “스쿼트”, “레그프레스”, “바벨 로우”, “오버헤드 프레스”, “데드리프트”];
...
const getWarmupSets = (weight, isLegPress) => {
if (isLegPress) {
return [
{ label: “웜업 1”, weight: Math.max(20, Math.round(weight * 0.4 / 10) * 10), reps: 10 },
{ label: “웜업 2”, weight: Math.max(20, Math.round(weight * 0.6 / 10) * 10), reps: 8 },
{ label: “웜업 3”, weight: Math.max(20, Math.round(weight * 0.8 / 10) * 10), reps: 5 },
];
}
return [
        ...
        {!session ? (
          <div>
            ...
            <div style={{ background: "#1a1a1a", borderRadius: 14, padding: 20, marginBottom: 16, border: "2px solid #2e2e2e" }}>
              ...
              <div style={{ border: "1.5px solid #333", borderRadius: 12, overflow: "hidden" }}>
                {getWorkouts(lowerChoice || "squat")[nextWorkout].map((ex, idx, arr) => {
                  const w = weights[ex.name] || DEFAULT_WEIGHTS[ex.name];
                  const warmups = getWarmupSets(w, ex.isLegPress);
                  return (
            ...
            {lowerChoice === null ? (
              <div style={{ background: "#1a1a1a", borderRadius: 14, padding: 20, border: "1px solid #2a2a2a" }}>
                ...
                <div style={{ display: "flex", gap: 10 }}>
                  ...
                  <button onClick={() => setLowerChoice("legpress")}
                    style={{ flex: 1, padding: "16px 8px", background: "#111", border: "2px solid #2a2a2a", borderRadius: 12, color: "#f0ede8", fontSize: 13, fontWeight: 600, cursor: "pointer", lineHeight: 1.6, textAlign: "center" }}>
                    레그프레스
                  </button>
                </div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                <button onClick={() => startSession(lowerChoice)}
                  ...
                  💪 운동 시작 ({lowerChoice === "squat" ? "스쿼트" : "레그프레스"})
                </button>
          ...
          <div>
            ...
            {session.exercises.map((ex, exIdx) => {
              const warmupSets = getWarmupSets(ex.weight, ex.isLegPress);
                          ...
                          return (
                            <div key={wi} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "7px 0", borderBottom: wi < warmupSets.length - 1 ? "1px solid #1a1a1a" : "none" }}>
                              <div>
                                ...
                                {ex.isLegPress && ws.weight > 0 && <span style={{ fontSize: 11, color: "#6db8e8", marginLeft: 4 }}>(한 손 {ws.weight / 2}kg)</span>}
                              </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 24 }}>
              {Object.entries(weights).map(([name, w]) => (
                <div key={name} style={{ background: "#1a1a1a", borderRadius: 12, padding: "12px 14px", border: "1px solid #2a2a2a" }}>
                  ...
                  {name === "불가리안 스플릿 스쿼트" && <div style={{ fontSize: 11, color: "#6db8e8", marginTop: 2 }}>한 손 {w / 2}kg · 더블 프로그레션</div>}
      ...
      <div>
        ...
        <div style={{ background: "linear-gradient(135deg, #0f1a2e, #1a1a1a)", borderRadius: 14, padding: 20, border: "1px solid #6db8e822", marginBottom: 16 }}>
          ...
          <div style={{ background: "#111", borderRadius: 10, padding: 14, border: "1px solid #6de8a022" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {[
                { emoji: "🦵", text: "스쿼트 랙이 없어도 OK — 레그프레스로 동일하게 5x5 선형 증가 가능해요" },
                { emoji: "📱", text: "매 세션 하체 운동을 자유롭게 선택할 수 있어요" },
                { emoji: "💜", text: "메인 완료 후 보조운동 5가지로 부족한 근육을 추가 자극해요 (불가리안 포함)" },
          ...
          <div style={{ background: "#111", borderRadius: 10, padding: 12 }}>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              <div style={{ flex: 1, background: "#1a1a1a", borderRadius: 8, padding: "8px 12px", textAlign: "center" }}>
                ...
                <div style={{ fontSize: 10, color: "#555", marginTop: 3 }}>5세트 x 5회 · 선형 증가</div>
              </div>
              <div style={{ flex: 1, background: "#1a1a1a", borderRadius: 8, padding: "8px 12px", textAlign: "center" }}>
                <div style={{ fontSize: 12, fontWeight: 700, color: "#6de8a0" }}>레그프레스</div>
                <div style={{ fontSize: 10, color: "#555", marginTop: 3 }}>5세트 x 5회 · 선형 증가</div>
              </div>
            </div>
          </div>
        ...
        <div style={{ background: "#1a1a1a", borderRadius: 14, padding: 20, border: "1px solid #2a2a2a", marginBottom: 16 }}>
          ...
          {[
            ...
            { name: "레그프레스", emoji: "🦵", point: "허리를 등받이에 밀착, 발뒤꿈치로 밀어내며 무릎 완전히 잠그지 않기", target: "대퇴사두 · 둔근 · 햄스트링" },
        ...
        <div style={{ background: "#1a2e1a", borderRadius: 14, padding: 20, border: "1px solid #2a4a2a", marginBottom: 16 }}>
          ...
          {[
            ...
            { emoji: "📌", title: "불가리안 중량 입력 주의", desc: "보조운동 불가리안은 양손 합산 중량을 입력해요. 한 손에 10kg이면 20kg으로 입력!" },
          ].map((tip, i) => (
